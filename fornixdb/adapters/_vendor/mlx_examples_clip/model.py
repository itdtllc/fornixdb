...
#
# Vendored verbatim (unmodified) from Apple's mlx-examples (MIT), clip/model.py:
#   https://github.com/ml-explore/mlx-examples/blob/main/clip/model.py
# See the LICENSE file in this directory. Runtime deps are mlx only.
#
# fornixdb does NOT call CLIPModel.from_pretrained (it targets the current
# checkpoint layout). Loading + the mlx-community layout remap live in the
# host adapter (fornixdb/adapters/mac_vision.py), so this file stays verbatim.
