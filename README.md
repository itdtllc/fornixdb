...
**Where this is heading** — FornixDB's direction is a climb along one axis: how tightly, how often, and how much in parallel memory is fused into the act of thinking, from "the program must ask" up toward memory that activates itself across many domains at once and steers the next thought. Today it sits **solidly at automatic capture (write-side autonomy), with proactive recall injection in hand but not yet lived-in**. The full ladder — and why parallel, in-thought recall is the human-like target — is in [ROADMAP.md](ROADMAP.md).
...
```bash
pip install "git+https://github.com/itdtllc/fornixdb@v0.3.0"
```
