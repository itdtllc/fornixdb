...
| `remind_me(what, when, urgent?)` | **Prospective memory** — remembering to *remember*. Store an intention with a due time ("remind me tomorrow morning to talk to Joe about his vacation plans"); the host's natural heartbeat (each chat turn, a voice loop's idle tick, session start) polls `due()` and delivers it when the clock arrives — exactly once, `delivered_at` is the tombstone. `urgent=true` nags instead: re-delivered every `nag_interval_minutes` (default 5) up to `nag_max_attempts` (default 6); the host calls `ack()` on every owner turn — the owner responding with *anything* after a delivery is the acknowledgment (observable, never model-interpreted) — and an exhausted, unanswered urgent reminder resurfaces at next session start (`unacknowledged()`, which re-arms it). `when` is natural: "in 20 minutes", "friday at 3pm", ISO. The host owns the clock, the store owns the intentions — FornixDB never runs scheduler threads on the host's behalf. | `prospective.remind` / `due` / `ack` / `unacknowledged` / `upcoming` + `timeparse.parse_due` |
...
| `reality` | a live memory points at a file under this home that no longer exists | fix/supersede, or accept with `tag <id> reality-ok` (`untag` takes it back if you accepted too fast) |
| `chronic` | chronic push-noise: pushed ≥6× with zero downstream use (lifetime pulls reported, never exempting) — the floor already quiets these; the dream asks if they should keep living | `supersede` (or MCP `forget_memory`) if obsolete, `reproject` if mis-scoped, or accept with `tag <id> noise-ok` (reversible with `untag`) |
| `reproject` | mis-scoped: unscoped/suspect rows whose CONTENT points at a project — the other root of cross-project push noise | **Read the rows before writing them.** The worklist prints the best 15; `reproject --apply` writes every proposal above `--min-margin`, which on a lived-in store is far more than that and is not reliable at the low end — projects that share vocabulary attract each other's memories. Run `reproject` alone first to see the whole confident set, raise `--min-margin`, and treat `--undo` as expected rather than exceptional |
| `markdown_stale` | a markdown memory file's open PICKUP/NEXT/TODO block looks OVERTAKEN by a later episodic row carrying closure language on the same subject (cosine on stored vectors; needs the bridge's `native_dir`) — the flat file has no machinery for noticing it is stale, the store does | rewrite the file (a fresh save clears the flag), or accept a reviewed pair with `link <file-row> <session-row> --relation distinct`. `brief` prints a one-line pointer while an unreviewed flag stands; `config dream_markdown_stale off` disables |
| `gists` | gists that are too long / hash-heavy / just the start of detail | `set-gist` in place |
| `distill` | raw session rows worth a durable summary | `store` a semantic memory (or `set-gist` the row in place), then `tag` the session `distilled` — that tag, not the gist, is what takes it off the list |
